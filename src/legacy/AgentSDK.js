...
export class AgentWidget {
  constructor(config = {}) {
    console.log('🎯 AgentWidget v2.1.9 initialized with config:', config);
    // Merge user config with comprehensive defaults
    this.config = this.mergeWithDefaults(config);
    this.sdk = new AgentSDK(this.config); // ✅ Pass merged config to AgentSDK
    this.isOpen = false;
    this.isActive = false;
  ...
  updateMicButtonState(isListening) {
    ...
    if (isListening) {
      ...
      micButton.style.background = btn.activeColor;
      micButton.title = 'Click to stop listening';
      
      // Add pulsing animation if enabled
      if (this.config.animation.enablePulse) {
        micButton.style.animation = 'pulse 1.5s infinite';
      }
      
      // Change icon to stop icon
      const iconSize = this.getIconSize();
      micButton.innerHTML = this.generateStopIconHTML(iconSize);
    } else {
      micButton.classList.remove('active');
      micButton.style.background = btn.primaryColor;
      ...
      const iconSize = this.getIconSize();
      micButton.innerHTML = this.generateIconHTML(iconSize);
    }
  }
  ...
  getIconSize() {
    ...
    return iconSizes[this.config.icon.size] || 28;
  }

  generateStopIconHTML(size) {
    return `<svg viewBox="0 0 24 24" style="width: ${size}px; height: ${size}px; fill: white;">
